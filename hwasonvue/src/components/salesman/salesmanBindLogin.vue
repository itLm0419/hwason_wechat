<template>
  <div class="hMian">
    <headerpart :imgaddr="imgaddr0" :title="title0" :salesmanBind="salesmanBind"></headerpart>
    ...
    <ul class="healthBindId">
      <li class="clearfix" >
        ...
        <input type="text" v-model="cardNo" class="fr" placeholder="请输写正确卡号">
      </li>
      <li class="clearfix" >
        <img src="/static/img/nurse/mustIcon.png" class="fl" alt="">
        <span class="checkName fl">密码</span>
        <input type="password" v-model="password" class="fr" placeholder="请输写正确信息">
      </li>
      <li class="clearfix">
        <img src="/static/img/nurse/mustIcon.png" class="fl" alt="">
        <span class="checkName fl">姓名</span>
        <input type="text" v-model="name" class="fr" placeholder="请输入您的真实姓名"/>
      </li>
      <li class="clearfix removeBorder">
        <img src="/static/img/nurse/mustIcon.png" class="fl" alt="">
        <span class="checkName fl">身份证号</span>
        <input type="text" v-model="IDNo" class="fr" placeholder="请输入您真实证号"/>
      </li>
    </ul>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            next(){
                var vm = this;
                if(!vm.cardNo||!vm.password||!vm.name||!vm.IDNo){
                    vm.$messagebox({
                        title:"提示",
                        message:"请填写完整信息"
                    });
                ...
                axios.post('/wechat/healthRecord/register',
                    ...
                    if(response.data.result == "02"){
                        ...
                        window.location.href = "#/baseMsg?userId="+response.data.userId+"&role=02";
